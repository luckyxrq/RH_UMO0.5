...
void bsp_DetectActTest(uint8_t pinMapIndex)
{
	...
	switch(action)
	{
		...
		case 1:
		{
			if(bsp_GetRunTime() - delay >= ChargeTime)
			{
		...
		case 2:
		{
			if(bsp_GetRunTime() - delay >= (IntervalTime-ChargeTime)) 
			{
				action = 0 ;
			}
		}break;
...
void bsp_DetectAct(void)
{
	...
	switch(detectAct.action)
	{
		...
		case 0:
		{
			...
			detectAct.delay = bsp_GetRunTime();
			detectAct.action++;
		...
		case 1:
		{
			if(bsp_GetRunTime() - detectAct.delay >= ChargeTime)
			{
		...
		case 2:
		{
			if(bsp_GetRunTime() - detectAct.delay >= (IntervalTime-ChargeTime)) 
			{
				bsp_AWSetPinVal(PinMap[detectAct.pinMapIndex%PIN_MAP_MAX][1], AW_0); //先开接收，充电
				detectAct.delay = bsp_GetRunTime();
				detectAct.action++;
