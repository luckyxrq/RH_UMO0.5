...
void bsp_EdgewiseRun(void)
{
	...
	switch(edgewiseRun.action)
	{
		case 0:/*进入沿边模式，首先直走*/
		{
			...
			{
				edgewiseRun.ErlangGodStartTime = bsp_GetRunTime() ;
				bsp_EdgewiseRunStraightSlow();
			}
			else
			{
				if(bsp_GetRunTime() - edgewiseRun.ErlangGodStartTime >= 1000)
				{
		...
		case 1:/*如果发生了碰撞，碰撞后退*/
		{
			...
			if(edgewiseRun.collision != CollisionNone)
			{
				...
				edgewiseRun.delay = bsp_GetRunTime();
				edgewiseRun.action++;
			}
		...
		{
			if((bsp_GetRunTime() - edgewiseRun.delay >= 2000) || (bsp_GetCurrentBothPulse()-edgewiseRun.pulse)>=GO_BACK_PULSE)
			{
				bsp_RotateCCW();
		...
		case 5:
		{
			...
			/*判断下旋转了太久了*/
			if((bsp_GetRunTime() - edgewiseRun.delay)>= 3000 && 
