...
void bsp_FillReportFrame(void)
{
	...
	uint16_t adc1  = bsp_GetInfraRedAdcVoltage(IR0);
	uint16_t adc2  = bsp_GetInfraRedAdcVoltage(IR1);
	uint16_t adc3  = bsp_GetInfraRedAdcVoltage(IR2);
	uint16_t adc4  = bsp_GetInfraRedAdcVoltage(IR3);
	uint16_t adc5  = bsp_GetInfraRedAdcVoltage(IR4);
	uint16_t adc6  = bsp_GetInfraRedAdcVoltage(IR5);
	uint16_t adc7  = bsp_GetInfraRedAdcVoltage(IR6);
	uint16_t adc8  = bsp_GetInfraRedAdcVoltage(IR7);
	uint16_t adc9  = bsp_GetInfraRedAdcVoltage(IR8);
	uint16_t adc10 = bsp_GetInfraRedAdcVoltage(IR9);
...
//	if(bsp_CliffIsDangerous(CliffRight)) cliffstatus+=8;
	...
	reportFrame.machine_status = bsp_GetLedAppState();                                //机器状态
