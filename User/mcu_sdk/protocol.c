...
{
  /*"请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"*/
  ...
  mcu_dp_enum_update(DPID_DIRECTION_CONTROL,当前清扫方向); //枚举型数据上报;
  mcu_dp_enum_update(DPID_STATUS,当前设备状态); //枚举型数据上报;
  mcu_dp_value_update(DPID_RESIDUAL_ELECTRICITY,当前剩余电量); //VALUE型数据上报;
  mcu_dp_value_update(DPID_EDGE_BRUSH,当前边刷寿命); //VALUE型数据上报;
  mcu_dp_value_update(DPID_ROLL_BRUSH,当前滚刷寿命); //VALUE型数据上报;
  mcu_dp_value_update(DPID_FILTER,当前滤网寿命); //VALUE型数据上报;
  mcu_dp_bool_update(DPID_RESET_EDGE_BRUSH,当前边刷重置); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_RESET_ROLL_BRUSH,当前滚刷重置); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_RESET_FILTER,当前滤网重置); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_SEEK,当前寻找机器); //BOOL型数据上报;
...
static unsigned char dp_download_switch_handle(const unsigned char value[], unsigned short length)
{
  ...
  unsigned char ret;
  //0:关/1:开
  unsigned char switch_var;
  
  switch_var = mcu_get_dp_download_bool(value,length);
  if(switch_var == 0)
  {
    //开关关
  }
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_bool_update(DPID_SWITCH,switch_var);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
功能描述 : 针对DPID_RESET_EDGE_BRUSH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
功能描述 : 针对DPID_RESET_ROLL_BRUSH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
