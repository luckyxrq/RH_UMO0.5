...
/******************************************************************************
                            1:修改产品信息                
******************************************************************************/
#define PRODUCT_KEY "fema6sntixpaypuh"    //开发平台创建产品后生成的16位字符产品唯一标识
...
******************************************************************************/
//开关(可下发可上报)
//备注:【非必选】 "可选择无开关机功能。 开关机控制。 下发： 发送控制指令给主机，主机依据指令状态进行开关机操作。 上报： 当主机使用遥控器关机或者长时间静止进入关机状态后，主机会上报一个关机数据。 当主机处于关机状态时，通过遥控器或者触摸主机的按键，主机会进入开机状态，主机会上报一个开机数据。"
#define DPID_SWITCH 1
//清扫开关(可下发可上报)
//备注:【必选】
#define DPID_SWITCH_GO 2
//工作模式(可下发可上报)
//备注:【必选】枚举值按需选择，不需要的可删除 standby：待机模式，random：随机模式，smart：自动清扫， wall_follow：沿边清扫 ，mop:拖地功能 ， spiral:定点清扫，left_spiral:左螺旋，right_spiral:右螺旋，right_bow：右⼸弓字，left_bow：左⼸弓字，partial_bow：局部弓字, chargego:回充模式
#define DPID_MODE 3
//清扫方向(可下发可上报)
//备注:【必选】 "前进，后退，左转，右转，停止。 默认逻辑：按住方向键下发 前/后/左转/右转，手指抬起下发停止"
#define DPID_DIRECTION_CONTROL 4
//设备状态(只上报)
//备注:状态指示 0：待机中 1：自动清扫 2：沿边清扫 3：定点清扫 4：拖地 5：回充中 6：充电中 7：充电完成8：暂停
#define DPID_STATUS 5
//剩余电量(只上报)
//备注:
#define DPID_RESIDUAL_ELECTRICITY 6
//边刷寿命(只上报)
//备注:
#define DPID_EDGE_BRUSH 7
//滚刷寿命(只上报)
//备注:
#define DPID_ROLL_BRUSH 8
//滤网寿命(只上报)
//备注:
#define DPID_FILTER 9
//边刷重置(可下发可上报)
//备注:
#define DPID_RESET_EDGE_BRUSH 10
//滚刷重置(可下发可上报)
//备注:
#define DPID_RESET_ROLL_BRUSH 11
//滤网重置(可下发可上报)
//备注:
#define DPID_RESET_FILTER 12
//寻找机器(可下发可上报)
//备注:
#define DPID_SEEK 13
//吸力选择(可下发可上报)
...
//清扫记录(只上报)
//备注:可选择将地图ID号同时上报 清扫日期+ YYYYMMDDTTRR 清扫时长 + XXX 清扫面积 xxx 如： "20180411051102008000020" 代表2018年4月11日05点11分，清扫了20分钟，80平方米 ，id号为20（假如MCU没有RTC，时间可不上报。记录则默认显示为数据上报时间）
...
//故障告警(只上报)
//备注:1边扫异常；2中扫异常；3左轮异常；4右轮异常；5垃圾盒异常；6 地检异常；7 碰撞异常； 当主机上报后，显示错误代码。
#define DPID_FAULT 18
//地图参数配置(只上报)
//备注:【有地图则必选】如果扫地机有地图功能，需要选择此功能点，用于配置地图参数，具体格式如下： 共3个字节,第一个字节为原点（0,0）位置，二三字节为地图最大的长宽 0x00 左上角 0x01 左下角 例：0x0000ff 左上角，最大长宽值为255
